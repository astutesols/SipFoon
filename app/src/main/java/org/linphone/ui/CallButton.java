		...
		try {
			if (!LinphoneManager.getInstance().acceptCallIfIncomingPending()) {
				if (mAddress.getText().length() > 0) {
					...
					if(LinphonePreferences.instance().getVoiceMailUri()!= null && mAddress.getText().toString().equalsIgnoreCase(LinphonePreferences.instance().getVoiceMailUri())){
					...
					android.util.Log.e("DIALER", "OUTGOING CALL ELSE");
			...
			android.util.Log.e("Contact details","LinphoneCoreException");
			e.printStackTrace();
