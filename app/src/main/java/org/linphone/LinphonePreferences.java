...
public class LinphonePreferences {
	...
	public void logoutAndExitUser(Context ctx) {
		getConfig().setBool("app", "first_launch", true);
		LinphonePreferences.instance().deleteAccount(LinphonePreferences.instance().getDefaultAccountIndex());
		LinphoneService.instance().sendNotification(3, R.string.notification_started);
		SharedPreferences mSharedPreferences = PreferenceManager.getDefaultSharedPreferences(ctx);
		mSharedPreferences.edit().remove(Config.PREF_IS_LOGIN_SUCCESS).commit();

		LinphoneService.instance().removeMessageNotification();
		Intent loginIntent = new Intent(ctx, LinphoneActivity.class);
