...
public class AccountPreferencesFragment extends PreferencesListFragment {
	...
	private void manageAccountPreferencesFields(PreferenceScreen parent) {
		...
		//displayName.setSummary(mPreferences.getString(Config.PREF_DISPLAY_NAME, ""));
		...
		//To hide account preferences
		advanced.removePreference(outboundProxy);
		advanced.removePreference(prefix);
		advanced.removePreference(avpf);
		advanced.removePreference(avpfRRInterval);
		advanced.removePreference(escape);
		manage.removeAll();
		getPreferenceScreen().removePreference(manage);
	}
