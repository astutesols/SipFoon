...
public class ApiNinePlus {
    ...
    public static List<String> extractContactNumbersAndAddresses(String id, ContentResolver cr) {
        ...
        // Phone Numbers
        Cursor c = cr.query(Phone.CONTENT_URI, new String[]{Phone.NUMBER}, Phone.CONTACT_ID + " = ? ", new String[]{id}, null);
        if (c != null) {
            while (c.moveToNext()) {
                String number = c.getString(c.getColumnIndex(Phone.NUMBER));
                list.add(number);
//                Log.e("extractContact ", " start num: " + number);
            }
            c.close();
        }
...
//                Log.e("extractContact ", " start sip: " + c.getString(nbid));
            }
            c.close();
        }
