...
//    }
    ...
    public static Cursor getGeneralContactCursor(ContentResolver cr, String select, boolean shouldGroupBy) {
        ...
        Cursor cursor = cr.query(Data.CONTENT_URI, projection, query, null, " lower(" + Data.DISPLAY_NAME + ") COLLATE NOCASE ASC");
...
//    }
    ...
    public static Contact getContact(ContentResolver cr, Cursor cursor, int position) {
        try {
            ...
            //added by Rajesh Jadav
            String phone_number = getContactNumber(cr, name);
            Uri thumbnail = getContactPictureUri(id);
            Uri photo = getContactPhotoUri(id);
            InputStream input;
            input = getContactPictureInputStream(cr, id);
            if(input == null){
                input = getContactPictureInputStreamLowQuality(cr, id);
            }

            Log.e("CONTACT","INPUT: "+String.valueOf(input==null));
            Contact contact;
            if (input == null) {
                ...
                contact = new Contact(id, name, phone_number, photo, thumbnail, bm);
            }
    ...
    public static InputStream getContactPictureInputStream(ContentResolver cr, String id) {
        InputStream is = openDisplayPhoto(cr, Long.parseLong(id));
        return is;
    }

    public static InputStream getContactPictureInputStreamLowQuality(ContentResolver cr, String id) {
    ...
    public static InputStream openDisplayPhoto(ContentResolver cr,long contactId) {
    ...
    private static String getContactNumber(ContentResolver cr,String name) {
        String number = "";
